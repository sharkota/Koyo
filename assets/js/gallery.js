...
function gallery_right() {
    ...
    for (let I = 0; I < gallery_images.length; I++) {
        if (gallery_images[I].src == current_src) {
            if (I == gallery_images.length - 1) {
                DisplayImage(gallery_images[0].src)
            } else {
                DisplayImage(gallery_images[I+1].src)
            }
        }
    }
}
...
function gallery_left(){
    ...
    for (let I = 0; I < gallery_images.length; I++) {
        if (gallery_images[I].src == current_src) {
            if (I == 0) {
                DisplayImage(gallery_images[gallery_images.length - 1].src)
            } else {
                DisplayImage(gallery_images[I-1].src)
            }
        }
